...
function TerrainEdit() {
  ...
  const fetchTerrain = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/terrains/${id}`);
      setFormData({
        nom: response.data.nom,
        description: response.data.description,
        visuels: response.data.visuels || [],
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/terrains/${id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
      ...
      setError(error.response?.data?.message || 'Erreur lors de la modification du terrain');
    } finally {
      setLoading(false);
    }
  };
